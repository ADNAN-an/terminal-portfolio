...
const BootScreen: React.FC = () => {
  ...
  const isFadingOut = bootState.stage === 'fade-out';

  return (
    <div className={`fixed inset-0 z-50 bg-black flex items-center justify-center transition-all duration-1000 ${
      isFadingOut ? 'opacity-0 scale-105' : isTransitioning ? 'transform scale-110 opacity-90' : ''
    }`}>
      {/* CRT Monitor Frame */}
      <div className={`relative transition-all duration-1500 ${
        isFadingOut ? 'transform scale-90 opacity-0' : isTransitioning ? 'transform scale-95 rotate-1' : ''
      }`}>
        {/* Monitor Bezel */}
        <div className={`bg-gray-800 p-8 rounded-3xl shadow-2xl transition-all duration-1000 ${
          isFadingOut ? 'bg-gray-900 shadow-4xl opacity-0' : isTransitioning ? 'bg-gray-900 shadow-4xl' : ''
        }`}>
          <div className={`bg-gray-900 p-4 rounded-2xl transition-all duration-1000 ${
            isFadingOut ? 'bg-black opacity-0' : isTransitioning ? 'bg-black' : ''
          }`}>
            {/* Screen */}
            <div 
              className={`
                ...
                ${isFadingOut ? 'border-green-500/20 shadow-sm shadow-green-500/10 opacity-0' : 
            ...
            >
              ...
              <div className={`absolute inset-0 scanlines pointer-events-none transition-opacity duration-1000 ${
                isFadingOut ? 'opacity-25' : isTransitioning ? 'opacity-75' : ''
              }`}></div>

              {/* Boot content */}
              {bootState.stage !== 'power-on' && (
                <div className={`p-8 font-mono text-green-400 text-sm leading-relaxed transition-all duration-1000 ${
                  isFadingOut ? 'transform translate-y-4 opacity-0' : isTransitioning ? 'transform translate-y-2 opacity-80' : ''
                }`}>
                  {displayedLines.map((line, index) => (
                    <div 
                      key={index} 
                      className={`boot-text mb-1 transition-all duration-500 ${
                        isFadingOut ? 'text-green-200 opacity-50' : isTransitioning ? 'text-green-300' : ''
                      }`}
                      style={{ 
                  ...
                  {/* Cursor */}
                  {(bootState.stage === 'loading' || bootState.stage === 'transition') && !isFadingOut && (
                  ...
                  {/* Fade out message */}
                  {isFadingOut && (
                    <div className="mt-4 text-center text-green-200 opacity-50">
                      <div className="text-lg">Loading Terminal...</div>
                    </div>
                  )}
                </div>
              )}
              ...
              <div className={`absolute inset-0 bg-gradient-radial from-green-500/5 via-transparent to-transparent pointer-events-none transition-all duration-1000 ${
                isFadingOut ? 'from-green-500/2' : isTransitioning ? 'from-green-500/10' : ''
              }`}></div>
          ...
          <div className="flex justify-center mt-4 space-x-4">
            <div className={`w-3 h-3 bg-red-500 rounded-full shadow-lg transition-all duration-500 ${
              isFadingOut ? 'opacity-50' : isTransitioning ? 'animate-pulse' : ''
            }`}></div>
            <div className={`w-3 h-3 bg-green-500 rounded-full shadow-lg animate-pulse transition-all duration-500 ${
              isFadingOut ? 'bg-green-400 shadow-green-400/30 opacity-50' : isTransitioning ? 'bg-green-400 shadow-green-400/50' : ''
            }`}></div>
            <div className={`w-8 h-3 bg-gray-600 rounded-full transition-all duration-500 ${
              isFadingOut ? 'bg-gray-500 opacity-50' : isTransitioning ? 'bg-gray-500' : ''
            }`}></div>
          </div>
        </div>
        ...
        <div className="absolute -bottom-2 left-1/2 transform -translate-x-1/2">
          <div className={`w-4 h-4 bg-green-500 rounded-full animate-pulse shadow-lg shadow-green-500/50 transition-all duration-500 ${
            isFadingOut ? 'bg-green-400 shadow-green-400/30 opacity-50' : isTransitioning ? 'bg-green-400 shadow-green-400/70' : ''
          }`}></div>
        </div>
      </div>
      ...
      <div className={`absolute inset-0 bg-gradient-radial from-green-900/20 via-transparent to-black pointer-events-none transition-all duration-1000 ${
        isFadingOut ? 'from-green-800/10' : isTransitioning ? 'from-green-800/30' : ''
      }`}></div>

      {/* Transition particles */}
      {(isTransitioning || isFadingOut) && (
        <div className="absolute inset-0 pointer-events-none">
          <div className={`particle particle-1 ${isFadingOut ? 'opacity-50' : ''}`}></div>
          <div className={`particle particle-2 ${isFadingOut ? 'opacity-50' : ''}`}></div>
          <div className={`particle particle-3 ${isFadingOut ? 'opacity-50' : ''}`}></div>
          <div className={`particle particle-4 ${isFadingOut ? 'opacity-50' : ''}`}></div>
          <div className={`particle particle-5 ${isFadingOut ? 'opacity-50' : ''}`}></div>
        </div>
      )}
    </div>
  );
